...
public class ChatHandler implements Listener {
    ...
    @Getter private Piston instance;
    ...
    private void onAsyncChat(AsyncPlayerChatEvent event) {
        ...
        if (!player.hasPermission("neutron.staff")) {
            ...
            if (this.isFiltered(msg)) {
                Piston.getInstance().getServer().getOnlinePlayers().stream().filter(it -> it.hasPermission("neutron.staff")).forEach(it -> it.sendMessage(ChatColor.translateAlternateColorCodes('&', "&c[Filtered] &f" + NeutronConstants.formatChatDisplay(player, msg))));
            ...
            if (this.tier2Filter.stream().anyMatch(msg::contains) || msg.equalsIgnoreCase("L")) {
                Piston.getInstance().getServer().dispatchCommand(Piston.getInstance().getServer().getConsoleSender(), "mute " + player.getName() + " 1h Toxicity (Said: " + msg + ") -p");
                event.setCancelled(true);
                return;
            }

            if (this.tier3Filter.stream().anyMatch(msg::contains)) {
                Piston.getInstance().getServer().dispatchCommand(Piston.getInstance().getServer().getConsoleSender(), "mute " + player.getName() + " 1w Toxicity (Said: " + msg + ") -p");
                event.setCancelled(true);
            }
        }
